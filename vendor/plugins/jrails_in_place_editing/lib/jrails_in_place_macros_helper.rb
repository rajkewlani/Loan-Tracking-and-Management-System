module InPlaceMacrosHelper
  ...
  def in_place_editor_field(object, method, tag_options = {}, in_place_editor_options = {})
    ...
    if object.kind_of?(ActiveRecord::Base)
      ...
      if tag.column_type.to_s == "date"
        if in_place_editor_options[:format].blank?
          in_place_editor_options[:format] = "%B %d, %Y"
        end
        val = tag.value(tag.object).strftime(in_place_editor_options[:format])
        field_html = tag.content_tag(tag_options.delete(:tag), val, tag_options)
      elsif in_place_editor_options[:format] == "currency"
        val = ActionController::Base.helpers.number_to_currency(tag.value(tag.object))
        field_html = tag.content_tag(tag_options.delete(:tag), val, tag_options)
      elsif in_place_editor_options[:field_type] == "select" && in_place_editor_options[:select_options].is_a?(Array)
        opts = Hash.new
        in_place_editor_options[:select_options].each do |opt|
          spl = opt.split(":")
          if spl.length == 2
            opts[spl[1]] = spl[0]
          else
        ...
        val = opts[tag.value(tag.object).to_s]
        field_html = tag.content_tag(tag_options.delete(:tag), val, tag_options)
      elsif in_place_editor_options[:format] == "phone"
        val = number_to_phone(tag.value(tag.object), :area_code => true)
        field_html = tag.content_tag(tag_options.delete(:tag), val, tag_options)
      else
